...
param_labels = ["sigma", "longitude", "latitude"]
...
for param, param_label, ax in zip(params, param_labels, axs.ravel()):
    chi2_value = chi2_parameters(f'{param}')
    values, bins, _ = ax.hist(chi2_value, bins=40, density=True, color=cb_color1,
                              label=f'Simulations={len(delta_ts)}',
                             edgecolor='black', alpha=0.4)
    centers = 0.5*(bins[:-1]+bins[1:])
    ax.plot(centers, chi2.pdf(centers, df=df_chi2), color=cb_color2, linestyle='--', linewidth=2)
    ax.legend(loc='upper right', fontsize=11)
    ax.text(0.61, 0.8, '$\chi^2$ function', verticalalignment='bottom',
            transform=ax.transAxes, horizontalalignment='left', color=cb_color2, fontweight='bold', fontsize=11)
    ax.text(0.61, 0.8, f'df = {df_chi2}',  
            verticalalignment='top', transform=ax.transAxes, horizontalalignment='left')
    ax.set_ylabel('Probability Density', fontsize=11)
    ax.set_xlabel(f'$\chi_p^2$ ({param_label})', labelpad=-1, fontsize=11)
for ax in axs:
    for spine in ax.spines.values():
        spine.set_linewidth(1.5)    
fig.savefig(paths.figures / 'chi2_mock_3ebands.pdf', bbox_inches='tight')        
